...
class KasbonController extends Controller
{
    ...
    public function index()
    {
        ...
        if (request()->has(['status'])) {
            $kasbons = $kasbons->where('status_kasbon', request('status'));
        }
    ...
    public function store(Request $request)
    {
        ...
        return to_route($this->base_route . 'transaksi.kasbons.edit', $kasbon->id)->with('success', 'Kasbon created.');

        // return to_route('admin.transaksi.kasbons.index')->with('success', 'Kasbon created.');
    }
    ...
    public function edit(Kasbon $kasbon)
    {
        ...
        return Inertia::render('Admin/Kasbon/Edit', [
            'kasbon' => new KasbonCollection($kasbon),
            'instansi' => $instansi,
            'statuskasbonOpts' => $xstatus_kasbons,
    ...
    //     return to_route('admin.transaksi.kasbons.index')->with('success', 'Kasbon Updated');
    // }
    ...
    public function getTotalKasbon()
    {
        ...
        $totalKasbon = Dkasbon::where('transpermohonan_id', '=', $transpermohonan_id)->sum('jumlah_biaya');
        return number_format($totalKasbon);
    }
    public function list()
    {
        $transpermohonan_id = request('transpermohonan_id');
        $dkasbons = Dkasbon::query();
        $dkasbons = $dkasbons->with('itemkegiatan:id,nama_itemkegiatan')
            ->with('kasbon', function ($q) {
        ...
        $dkasbons = $dkasbons->orderBy('dkasbons.id', 'desc')
            ->cursorPaginate(10)->withQueryString();
        return DkasbonCollection::collection($dkasbons);
        // return $dkasbons;
    }
