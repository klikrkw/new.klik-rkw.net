...
class Dkasbon extends Model
{
    ...
    public static function getPrimaryId()
    {
        ...
        $month = $now->month;
        $date = $now->day;
        $rec = Dkasbon::select('id')->whereDate('created_at', '=', $now->toDateString())->orderBy('id', 'desc')
            ->skip(0)->take(1)->first();
    ...
    {
        parent::boot();
        self::creating(function (Dkasbon $dkasbon) {
            $dkasbon->id = Dkasbon::getPrimaryId();
        });
    }
