...
class Dkasbonnoperm extends Model
{
    use HasFactory;
    ...
    public static function getPrimaryId()
    {
        ...
        $month = $now->month;
        $date = $now->day;
        $rec = Dkasbonnoperm::select('id')->whereDate('created_at', '=', $now->toDateString())->orderBy('id', 'desc')
            ->skip(0)->take(1)->first();
    ...
    {
        parent::boot();
        self::creating(function (Dkasbonnoperm $dkasbon) {
            $dkasbon->id = Dkasbonnoperm::getPrimaryId();
        });
    }
