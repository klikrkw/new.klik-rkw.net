...
export default function PermohonanSelect({
    className,
    onValueChange,
    value,
    errors,
    ...
    const { data: filteredOption, error } = useSWR<Permohonan[]>(
        ["/admin/permohonans/api/list/", query],
        ...
        <>
            ...
            >
                <div className={twMerge(`relative`, className)}>
                    <div
                        className={twMerge(
                            "relative overflow-visible flex w-full cursor-default bg-white text-left rounded-lg shadow-md focus:outline-none focus-visible:ring-2 focus-visible:ring-white/75 focus-visible:ring-offset-2 focus-visible:ring-offset-teal-300 sm:text-sm z-50",
                            className
                        )}
                    >
                        <ListboxSelect
                        ...
                        <Combobox.Input
                            ...
                            displayValue={(permohonan: Permohonan) =>
                    ...
                    <Transition
                        ...
                        afterLeave={() => setQuery("")}
                    >
                        <Combobox.Options className="absolute mt-1 max-h-60 w-full md:w-[100vh] overflow-auto rounded-md bg-white py-0 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-xs z-50">
                            {filteredOption && filteredOption.length > 0 ? (
                                <div
                                    className={`flex flex-row p-2 w-full font-bold bg-lightBlue-800 text-lightBlue-300 border-lightBlue-700 `}
                                >
                            ...
                            query !== "" ? (
                                ...
                                filteredOption &&
                                filteredOption.map((permohonan: Permohonan) => (
                                    <Combobox.Option
                                        key={permohonan.id}
                                        className={({ active }) =>
