...
export default function ListboxSelect({ className, listOptions, onListChange }: Props) {
    const [selected, setSelected] = useState<OptionSelect>(listOptions[0])
                ...
                <div className="relative">
                    ...
                    >
                        <Listbox.Options className="absolute mt-1 max-h-60 w-full overflow-auto rounded-md bg-white py-1 text-base shadow-lg ring-1 ring-black/5 focus:outline-none sm:text-sm">
                            {listOptions.map((listOption: OptionSelect, listOptionIdx: number) => (
                                <Listbox.Option
                                    key={listOptionIdx}
                                    className={({ active }) =>
