...
export default function CardTableRoles({ color = "light", roles, className = "", meta, labelLinks }: { color: "dark" | "light", roles: Role[], className?: string, meta: { links: [], per_page: number, total: number }, labelLinks: any }) {
    ...
    return (
        <>
            <div
                className={
                    twMerge("relative flex flex-col min-w-0 break-words w-full mb-6 shadow-slate-700 shadow-md rounded-lg ",
                        (color === "light" ? "bg-white" : "bg-lightBlue-900 text-white"), className)
                }
            >
                <div className="rounded-t mb-0 px-4 py-3 border-0">
                    <div className="text-center flex justify-between">
                        <h1 className={"text-blueGray-700 text-lg font-bold uppercase " + (color === "light" ? "text-blueGray-700" : "text-white")}>Roles</h1>
                        <LinkButton theme="blue" href={route('admin.roles.create')} >
                            New Role
                        </LinkButton>
                    </div>
                </div>
                        ...
                        <tbody>
                            {
                                roles.map(({ id, name, permissions }, index) => (
                                    <tr key={index}>
                                        ...
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2">
                                            <i className="fas fa-circle text-orange-500 mr-2"></i> {name}
                                        </td>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2">
                                            <div>{permissions.map((v, i) => (<span key={i}>{i > 0 ? ', ' : ''}{v.name}</span>))}</div>
                                        </td>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2 ">
                                            <div className="flex justify-start gap-1 ">
                                                <Link href={route('admin.roles.edit', id)} className="text-lightBlue-500 background-transparent font-bold px-3 py-1 text-xs outline-none focus:outline-none hover:text-lightBlue-100 hover:scale-105 mr-1 mb-1 ease-linear transition-all duration-150" type="button">
                                                    Edit
                                                </Link>
                                                <button
