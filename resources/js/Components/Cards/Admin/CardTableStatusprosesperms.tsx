...
export default function CardTableStatusprosesperms({ color = "light", statusprosesperms, className = "", meta, labelLinks }: { color: "dark" | "light", statusprosesperms: Statusprosesperm[], className?: string, meta: { links: [], per_page: number, total: number }, labelLinks: any }) {
                ...
                <div className="rounded-t mb-0 px-4 py-3 border-0">
                    <div className="flex justify-between">
                        ...
                        <div className="flex justify-center gap-2 flex-row items-start">
                            ...
                            >
                                <span>New</span>
                                <span className="hidden md:inline"> Status Proses</span>
                            </LinkButton>
                        </div>
                    </div>
                </div>
                    ...
                    <table className="items-center w-full bg-transparent border-collapse">
                        <thead>
                            <tr>
                                ...
                                >
                                    <Link href="#" onClick={(e) => handleSortLinkClick({ sortBy: 'nama_statusprosesperm', sortDir: values.sortDir === 'asc' ? 'desc' : 'asc' })} >
                                        <div className="flex flex-row justify-between">
                                            <span>Nama Item Proses Permohonan</span>
                                            <IconSort sortBy='nama_statusprosesperm' sortDir={values.sortDir || ''} />
                                        </div>
                                    </Link>
                                </th>
                                <th className={
                        ...
                        <tbody>
                            {
                                statusprosesperms.map(({ id, nama_statusprosesperm }, index) => (
                                    <tr key={index}>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2">
                                            {id}
                                        </td>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2">
                                            <i className="fas fa-circle text-orange-500 mr-2"></i> {nama_statusprosesperm}
                                        </td>
                                        <td className="border-t-0 px-6 align-middle border-l-0 border-r-0 text-xs whitespace-nowrap p-2 ">
                                            <div className="flex justify-start gap-1 ">
                                                <Link href={route('admin.statusprosesperms.edit', id)} className="text-lightBlue-500 background-transparent font-bold px-3 py-1 text-xs outline-none focus:outline-none hover:text-lightBlue-100 hover:scale-105 mr-1 mb-1 ease-linear transition-all duration-150" type="button">
                                                    Edit
                                                </Link>
                                                <button
